...
def build(out_path):
    ...
    ax[0].text(2, ci_hi * 1.05, f"{fp_m0 / fp_b:.1f}x fewer", ha="center",
    ...
    ax[1].set(ylabel="recall on the 0.15-0.30% stratum (%)", ylim=(0, 100),
    ...
    fig.suptitle("Head-to-head on the frozen test split "
                 f"(matched recall, {n_test} test-split files)", y=1.02,
                 fontsize=10.5)
    fig.tight_layout()
